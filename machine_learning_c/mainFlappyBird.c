...
Pipe closestPipe(Bird* bird, PipesStack* pipes)//return closest coming pipe to bird
{
	if (pipes->top == 0)
	{
		puts("0 pipes");
	...
	{
		if (pipes->stack[i].x - bird->x < distance && pipes->stack[i].x - bird->x > 10) // if is ahead of bird and closest
		{
...
void think(Bird* bird, PipesStack* pipes)//makes bird take action based on nn feed forward
{
	int inputSize = 5;
	double inputs[5];
...
int main(int argc, char **argv)
{
	...
	PipesStack *pipes = (struct PipesStack*)malloc(sizeof(PipesStack)); // allocate the pipes queue
	pipes->top = 0;
